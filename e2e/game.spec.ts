...
test('desktop: drag a piece to a legal square and cancel an invalid drop',async({page})=>{
  ...
  expect(await page.evaluate(key=>JSON.parse(localStorage.getItem(key)!).moves.length,SAVE_KEY)).toBe(2);
});
test('desktop: onboarding, legal moves, enemy reply, takeback, keyboard, save and help',async({page})=>{
  ...
  await page.waitForFunction(key=>JSON.parse(localStorage.getItem(key)!).moves.length===2,SAVE_KEY);
  const saved=await page.evaluate(key=>localStorage.getItem(key),SAVE_KEY);await page.reload();expect(await page.evaluate(key=>localStorage.getItem(key),SAVE_KEY)).toBe(saved);
...
test('desktop: takeback can rewind several turns in a row',async({page})=>{
  ...
  await page.waitForFunction(key=>JSON.parse(localStorage.getItem(key)!).moves.length===4,SAVE_KEY);
  const takeback=page.getByRole('button',{name:/Takeback/});
...
test('desktop: a pasted road code starts that exact road',async({page})=>{
  ...
  expect(await page.evaluate(key=>JSON.parse(localStorage.getItem(key)!).seed,SAVE_KEY)).toBe(1295);
...
test('complete act through the UI: fight, recruit, advance, defeat the boss',async({page})=>{
  ...
  for(let stage=0;stage<ENCOUNTERS.length;stage++){
    ...
    if(stage<ENCOUNTERS.length-1){
      ...
      const state:Run=await page.evaluate(key=>JSON.parse(localStorage.getItem(key)!),SAVE_KEY);
