  ...
  return <div className={`app theme-${encounter.theme}`}>
    ...
    <div className="layout">
      ...
      <main className="battle-panel">
        ...
        <div className="material" role="status" aria-label={`Material: ${swing.taken.length} taken, ${swing.lost.length} lost, ${swing.reading}`}>{swing.taken.length>0&&<span className="material-side">TAKEN {swing.taken.map((type,i)=><Piece key={i} type={type} color="b" small/>)}</span>}{swing.lost.length>0&&<span className="material-side">LOST {swing.lost.map((type,i)=><Piece key={i} type={type} color="w" small/>)}</span>}<strong className={swing.delta>0?'ahead':swing.delta<0?'behind':''}>{swing.reading}</strong></div>
