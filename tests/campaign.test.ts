...
it('can complete an entire campaign with legal moves, rewards, and recruitment',async()=>{
  ...
  for(let stage=0;stage<ENCOUNTERS.length;stage++){
    let plies=0;
    while(run.phase==='battle'&&plies<180){const chess=getChess(run);const move=chooseMove(chess,chess.turn()==='w'?'tactician':'wanderer',run.elite,chess.turn()==='w'?Math.max(run.stage,6):run.stage);expect(move).toBeTruthy();run=playMove(run,move!.san);plies++;}
